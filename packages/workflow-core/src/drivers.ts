...
export class WorkflowDrivers {
  ...
  public async executeHTTPStep(
    ...
    try {
      ...
      const response = await fetch(fullUrl, {
        ...
        headers: {
          ...opspilotHeaders(correlationId, config.workflowRunId, config.stepId),
          ...headers
        },
        body: requestBody,
      ...
      if (isNativeWs) {
        ...
        return new Promise((resolve) => {
          ...
          if (config.action === "emit") {
            ...
            const joinPayload = JSON.stringify({
              event: "join",
              room: config.room,
              opspilot: opspilotMetadata(correlationId, config.workflowRunId, config.stepId)
            });
        ...
        if (!socket) {
          ...
          socket = io(nspUrl, {
            auth: {
              ...(config.auth || {}),
              opspilotCorrelationId: correlationId,
              opspilotWorkflowRunId: config.workflowRunId,
              opspilotStepId: config.stepId
            },
            extraHeaders: {
              ...opspilotHeaders(correlationId, config.workflowRunId, config.stepId)
            },
  ...
  private async getBrowserSession(
    sessionId: string,
    correlationId: string,
    workflowRunId?: string,
    stepId?: string
    ...
    await page.route("**/*", async (route) => {
      await route.continue({
        headers: {
          ...
          ...opspilotHeaders(correlationId, workflowRunId, stepId)
        }
      });
    });
...
function opspilotHeaders(
  ...
  return Object.fromEntries(
    Object.entries({
      ...
      "x-opspilot-workflow-run-id": workflowRunId,
      "x-opspilot-step-id": stepId
    }).filter((entry): entry is [string, string] => typeof entry[1] === "string" && entry[1].length > 0)
  );
}

function opspilotMetadata(
  correlationId: string,
  workflowRunId?: string,
  stepId?: string
