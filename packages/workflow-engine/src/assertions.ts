...
export class AssertionEngine {
  ...
  public async assertQueueEvent(config: QueueAssertionConfig): Promise<AssertionResult> {
    ...
    try {
      const prefix = `bull:${config.queueName}`;
      const jobIds = config.jobId
        ? [config.jobId]
        : await scanJobIds(client, prefix, 200);
      for (const jobId of jobIds) {
        const job = await client.hgetall(`${prefix}:${jobId}`);
        if (!job || Object.keys(job).length === 0) continue;
        const payload = parseJsonRecord(job.data);
        ...
        const state = await bullMqJobState(client, prefix, jobId);
        if (config.state && state !== config.state) continue;

        return {
          success: true,
          log: `QUEUE_ASSERTION_PASSED: Found BullMQ job ${jobId} in ${config.queueName} with state ${state}.`,
          evidence: {
  ...
  private evaluateRows(
    ...
    return {
      ...
      log: rows.length
        ? `DATABASE_ASSERTION_PASSED: Read-only query matched ${rows.length} rows.`
...
function isJobId(value: string): boolean {
  return Boolean(value) &&
    !["meta", "events", "id", "marker", "stalled-check", "pc", "metrics"].some(
      (reserved) => value === reserved || value.startsWith(`${reserved}:`)
    );
}

async function bullMqJobState(
  client: RedisClient,
  prefix: string,
  jobId: string
): Promise<string> {
  for (const state of ["completed", "failed", "delayed"] as const) {
    if (await client.zscore(`${prefix}:${state}`, jobId) !== null) return state;
  }
  if (await client.lpos(`${prefix}:active`, jobId) !== null) return "active";
  if (
    await client.lpos(`${prefix}:wait`, jobId) !== null ||
    await client.lpos(`${prefix}:paused`, jobId) !== null
  ) return "waiting";
  if (await client.sismember(`${prefix}:stalled`, jobId)) return "stalled";
  return "unknown";
}
